...
- **Docs:** [DEVELOPMENT.md](DEVELOPMENT.md) engineering guide (from-source setup, CLI, AppData caches, Raid BiS class/stub hydration, HTTPS fetch allowlist, Type 18/19, Heroic AA, tests, Authenticode release pipeline). Troubleshooting notes for stale catalog / failed T-level caches.
...
- **Faster Generate Report:** Type 7/8 and Raid BiS catalogs reuse the disk cache instead of re-querying EQ Resource on every run. Failed gear T-level lookups are remembered so unknown items are not fetched again. Remaining live item lookups run in parallel.
...
- **App icon:** Windows executable, GUI header badge, HTML report logo/favicon, and the product page now use the circular EQGM emblem.
