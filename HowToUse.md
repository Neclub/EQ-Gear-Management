...
| Type 18/19 (or Type 7/8 / Raid BiS) looks stale after an EQ Resource update | Close EQGM. Delete the catalog JSON under `%LOCALAPPDATA%\EQGM\` (for Type 18/19: `eqresource_type18_catalog_cache.json` and `eqresource_type18_item_meta_cache.json`). Leave `settings.json` if you want to keep roster order and last EQ Folder. Regenerate. The app has no “refresh cache” button — later runs reuse those files. |
