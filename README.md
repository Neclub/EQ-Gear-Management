...
- **Type 18/19 Augs** — *(work in progress)* optional per-class suggestions from the Zarax cheat sheet (on by default); pick a character to set class and see **Owned** for that inventory; top unused Fortifications append to Optional; Enhancement augs under **Filler**; anniversary picks (Jubilation / Enduring Harmony) marked on the item name with non-anniversary alternatives; craft anvil copies non-anniversary names; full catalog view still available
