...
3. If Windows Defender flagged the old single-file `.exe`, use this zip instead. A single-file `.exe` is still on the release for older in-app update checks.

Windows SmartScreen may still warn on first run. Download only from this GitHub repo’s **Releases**. Choose **More info → Run anyway** if SmartScreen asks.
