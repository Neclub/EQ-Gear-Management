...
| `src/inventory_parser/http_fetch.py` | HTTPS GET/POST limited to EQ Resource and raidloot |
...
| `.github/workflows/build-release.yml` | Windows exe + optional Authenticode + GitHub Release on `main` |
...
**GUI:** `web_gui` hosts `data/gui/` in pywebview (`web_bridge.DEFAULT_WINDOW_WIDTH` × `DEFAULT_WINDOW_HEIGHT` = 982×765, dark-fantasy chrome) and calls `web_api.WebApi` (folder pickers, roster, tier colors, generate on a background thread). After generate, `open_html_report()` opens the saved file in the default browser only if the suffix is `.html` / `.htm`. The setup window stays open.
...
**Generate progress:** `on_progress` payloads include a status `message`. Type 7/8, Type 18/19, and Raid BiS name the live source (`Fetching from EQ Resource…`, `Fetching from raidloot.com…`) or a cache hit (`Using cached …`). Raid BiS also reports item-page and icon steps. Do not reuse a generic “catalog” string when the fetch is EQ Resource.
...
There is no `--force-refresh` (and `web_api` never passes `force_refresh`). Stale EQ Resource data is fixed by deleting cache files (below).
...
| `eqresource_aug_cache.json` | Type 7/8 / Type 5 item pages (`stats_v` must be ≥ `CACHE_STATS_VERSION` = 4) |
...
| `raid_bis_catalog.json` / `raid_bis_item_cache.json` | Raid BiS (`_version` must match `ITEM_CACHE_VERSION` in `raid_bis/catalog.py`, currently **7**) |
| `item_icons/` | Raid BiS HTML icons (PNG only; non-PNG cache files are ignored) |
...
Remaining live item lookups (gear T-level, Type 18/19 item meta, Raid BiS jewelry/item pages) run with a small thread pool (typically 6 workers).

---

## HTTPS fetches

Runtime catalog, item, and icon downloads go through `http_fetch.py`. Allowed hosts are `eqresource.com` (plus `www`, `items`, `sor`) and `raidloot.com` / `www.raidloot.com`. Requests must be HTTPS, port 443, with no userinfo; redirects that leave that set are refused. HTML bodies cap at 5 MB; item icons cap at 256 KB and must start with the PNG magic bytes.

`app_updates.py` is a separate allowlist: GitHub Releases API (`api.github.com`) and this repo’s HTTPS `EQGM-x.y.z.exe` asset on `github.com`. The app opens the download URL in the browser and never runs the file.

Achievement pages (`achievements.eqresource.com`) are **links in reports**, not runtime fetches. `scripts/enrich_heroic_aa_eqresource_ids.py` uses its own `urllib` and is not routed through `http_fetch`.

---

## Raid BiS internals

Flow: `raid_bis/catalog.fetch_catalog()` → `compare.py` scoring → Excel/HTML (`html.py` paperdoll + icons).

**Class lists (1.35.4):** `RaidGearCandidate.classes` is `None` (unknown → not wearable), an empty frozenset (`Class: All`), or the parsed abbreviations. `fits_class()` never treats unknown jewelry as All. Disk cache stores All as the full class list (`class_all: true`); a legacy empty `classes: []` is loaded as unknown so pet-focus ears are not handed to every class. Pet-focus ears (`Enhanced Minion` focus or `Summoner` in the name) stay MAG / BST / NEC (`PET_FOCUS_CLASSES`).

**Armor stubs (1.35.5):** `raidarmor.php` matrices first create names like `PAL Chest`. `_item_needs_page_hydrate()` is true when class is unknown, the name is still a class/slot stub (or `Item N`), icon is missing, or stats are empty. Cached stubs with a class list are still hydrated for real names, stats, and icons. Incomplete stubs are not written into `raid_bis_item_cache.json`. After updating, regenerate once so `ITEM_CACHE_VERSION` 7 class lists and hydrated armor rows replace older cache files.

**Progress:** cache hit → `Using cached Raid BiS catalog…`; live pages → `Fetching from EQ Resource…`; EQ Resource failure with no cache → `Fetching from raidloot.com…`; then item details and `item_icons/` PNG fetches.
...
Each catalog row stores whether that achievement **awards** Fortitude / Resolution / Vitality (0 or 1). HTML omits F/R/V chips when the rank is 0; Excel leaves those cells blank. Lit vs muted follows Completed / Incomplete, not the 0/1 award flags. Names link to EQ Resource when `eqresource_id` is set.
...
```powershell
py -3 -m pytest
py -3 -m pytest tests/test_type18_augs.py tests/test_heroic_aas.py tests/test_type5_augs.py tests/test_raid_bis.py tests/test_http_fetch.py
```
...
Type 18/19, Raid BiS, and HTTP tests use `tests/fixtures/` HTML (or in-process URL refusals) — do not add tests that hit the network.
...
3. Merge to `main`. [build-release.yml](.github/workflows/build-release.yml) runs on Windows, builds `dist/EQGM-<version>.exe` via `scripts/run_pyinstaller.py`, optionally Authenticode-signs, publishes GitHub Release `v<version>`, and posts Discord if `DISCORD_WEBHOOK_URL` is set.

**PyInstaller** (`scripts/run_pyinstaller.py`): `--onefile --noconsole --noupx`, embeds `src/inventory_parser/assets/eqgm.manifest` (`asInvoker`), and writes version info (company **Lubworks**, Comments = this GitHub repo URL).

**Signing:** local `build_exe.bat` calls `scripts/sign_exe.py` after the build. Copy `codesign.local.bat.example` → `codesign.local.bat` (gitignored) and set `IP_SIGN_PFX` / `IP_SIGN_PASSWORD` or `IP_SIGN_THUMBPRINT`. GitHub Releases signs when repo secrets `IP_SIGN_PFX_B64` (base64 of the `.pfx`) and `IP_SIGN_PASSWORD` are set; if `IP_SIGN_PFX_B64` is empty the workflow skips signing and still publishes. Unsigned PyInstaller builds can still trip SmartScreen heuristics.
...
- **Raid BiS recommends an item the class cannot wear** — pre-1.35.4 caches treated jewelry with no class list as All. Use 1.35.4+ and regenerate (item cache `_version` must be 7).
- **Raid BiS paperdoll shows `PAL Chest`** — pre-1.35.5 skipped hydrating armor rows that already had a class list. Use 1.35.5+ and regenerate.
...
- **Fetch to a new host fails** — add the hostname to `http_fetch.ALLOWED_HOSTS` only if generate-time code should call it; GitHub updates stay in `app_updates.py`.
