...
def test_crew_slots_visible_then_non_visible() -> None:
    assert CREW_GEAR_SLOTS[: len(VISIBLE_SLOTS)] == VISIBLE_SLOTS
    assert CREW_GEAR_SLOTS[len(VISIBLE_SLOTS) :] == NON_VISIBLE_SLOTS
...
def test_slots_for_export_filters() -> None:
    ...
    assert slots_for_export("all") == CREW_GEAR_SLOTS
